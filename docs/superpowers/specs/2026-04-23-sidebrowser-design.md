...
**纯 reducer 设计：**
`edge-dock-reducer.ts` 导出 `reduce(state, event) => { nextState, effects[] }`，effects 是枚举式副作用描述（`{type: 'APPLY_DIM'}` / `{type: 'ANIM_HIDE', from, to, ms}` 等）。`edge-dock.ts` 拿到 effects 后真实执行。reducer 100% 单测覆盖，不碰真实窗口。
...
SettingsDrawer slider onChange
  → invoke('settings:update', { dim: { blurPx: 16 } })
    ...
    → 若 DimController 正 active：removeInsertedCSS(oldKey) + insertCSS(newRule) 立即生效
...
v1 **不**注册全局快捷键（避免与用户其他软件冲突）。召唤应用只能通过托盘图标或触发条。
